...
function hasImpliedEarlierReferenceDate(result: ParsingResult): boolean {
    return result.text.match(/\s+(antes|de)$/i) != null;
}

function hasImpliedLaterReferenceDate(result: ParsingResult): boolean {
    return result.text.match(/\s+(depois|desde)$/i) != null;
}
 ...
 */
export default class PTMergeRelativeDateRefiner extends MergingRefiner {
